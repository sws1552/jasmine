...
.line0{
/* 	border-bottom-color: rgb(102, 102, 102); border-bottom-width: 0.5px; border-bottom-style: dotted; */
padding:5px;
}
.line1{
/* 	border-bottom-color: rgb(102, 102, 102); border-bottom-width: 0.5px; border-bottom-style: dotted; */
...
.line2{
	border-bottom-color: rgb(102, 102, 102); border-bottom-width: 0.5px; border-bottom-style: solid;
	padding:5px;
	
} 
.detail_table3{
	border-bottom-color: rgb(102, 102, 102); border-bottom-width: 1px; border-bottom-style: solid;
	border-left-color: rgb(102, 102, 102); border-left-width: 0.5px; border-left-style: solid;
	border-right-color: rgb(102, 102, 102); border-right-width: 0.5px; border-right-style: solid;
}
...
//	alert("총주문 금액:"+total_order_goods_price);
	...
	delivery_addr = i_roadAddress.value + i_jibunAddress.value;
	...
	var orderer_mail = document.getElementById("mail");
	console.log("p_order_goods_id >> "  + p_order_goods_id);
	console.log("receiver_name >> "  + receiver_name);
	console.log("p_receiver_tel_num >> "  + p_receiver_tel_num);
	console.log("delivery_addr >> "  + delivery_addr);
	console.log("i_roadAddress.value>> "  +i_roadAddress.value);
	console.log("i_jibunAddress.value >> "  + i_jibunAddress.value);
	console.log("i_zipcode.value >> "  + i_zipcode.value);
	console.log("orderer_mail >> "  + orderer_mail.value);
	
	var IMP = window.IMP; 
	IMP.init("imp15084481"); 
	
	 IMP.request_pay({ // param
         pg: "html5_inicis",
         pay_method: "card",
         merchant_uid: "Jasmine_" + new Date().getTime(),
         name: goods_title,
         amount: 100,
         buyer_email: orderer_mail.value,
         buyer_name: receiver_name,
         buyer_tel: receiver_hp_num,
         buyer_addr: delivery_addr,
         buyer_postcode: i_zipcode.value
...
<body>
	<H1 style="padding-top:20px;">1.주문확인</H1>
<form  name="form_order" style="height: 1333px;">	
	...
	<table width=80% class="list_view">
		<tbody>
			<tr align=center class="fixed2">
				<td class="tdf">총 상품수</td>
				<td class="tdf">총 상품금액</td>
				<td class="tdf"></td>
				<td class="tdf">총 배송비</td>
				<td class="tdf"></td>
				<td class="tdf">총 할인 금액</td>
				<td class="tdf"></td>
				<td class="tdf">최종 결제금액</td>
			</tr>
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_final_totalPrice">
						<font size="5">${final_total_order_price }원 </font>
		...
		<div style="position:relative; display:inline">
			...
			<button type="button" class="btn btn-outline-danger" id="btns" style="position:relative; margin-top :30x; margin-left:260px; margin-botton : 200px;">결제하기</button>
			
			</a> <a href="${contextPath}/main/main.do"> 
			  <button type="button" class="btn btn-outline-info" id="btns" style="position:relative;margin-top: 15px;margin-left:0px;margin-botton : 260px;">쇼핑계속하기</button></a>
			</a>
		</div>
	</div>
   <div class="clear"></div>
	...
	<!-- <h1 class="h1">4.결제정보</h1> -->
	<div class="detail_table3" style="visibility:hidden">
		<table>
			<tbody>
			<div class="dtline">
				<tr >
